...
Процедура ПодготовитьДанныеДляСозданияДокументовСервер(ПараметрыСозданияДокументов)
	...
	Для Каждого Параметр Из ПараметрыСозданияДокументов Цикл
		...
		Для Каждого Строка Из НайденныеМатериалы1 Цикл
			...
			//ЕстьМатериалы = Истина;
			////MSA++
			//СуммаСпецификации = СуммаСпецификации + (Строка.Цена * Строка.Количество);
			////MSA--
		КонецЦикла;
		...
		КонецЕсли;
		//Сообщить(СуммаСпецификации);
		Если ВыходныеИзделия1.Количество() = 0 И СуммаСпецификации > 0 Тогда
		...
		Если ЕстьМатериалы Тогда
			...
			Если Материалы1.Количество() И Материалы2.Количество() Тогда
				ВыходныеИзделия1.Добавить(СтрокаТоваров);
			ИначеЕсли Материалы1.Количество() Тогда
				ВыходныеИзделия.Добавить(СтрокаТоваров);
			КонецЕсли;
		КонецЕсли;
