...
Процедура СписокЗаказовПриИзменении(Элемент)
	...
	ЗаказITOkna = ЕстьЗаказITOkna(ТекущиеДанные.УникальныйИдентификатор);
	Если ЗначениеЗаполнено(ЗаказITOkna) Тогда
		ОчиститьСообщения();
		ТекстСообщения = СтрШаблон(Нстр(
		"ru = 'Заказ № %1 от %2 уже существует в базе'"),
		Формат(ТекущиеДанные.Номер, "ЧГ=0"),
		Формат(ТекущиеДанные.Дата, "ДЛФ=DD"));
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, ЗаказITOkna);
...
Процедура ПодготовитьДанныеДляСозданияДокументовСервер(ПараметрыСозданияДокументов)
	...
	"УникальныйИдентификатор,Артикул,Наименование,Декор,Цена,Количество");
	СпецификацииITOkna.Свернуть("УникальныйИдентификатор,Артикул,Наименование,Декор,Цена", "Количество");
	...
	Для Каждого Параметр Из ПараметрыСозданияДокументов Цикл
		...
		Для Каждого Строка Из НайденныеСтроки Цикл
			...
			Если Номенклатура = Материалы Тогда
				Если НайденныеМатериалы2.Количество() Тогда
					ВыходныеИзделия2.Добавить(СтрокаТоваров);
				ИначеЕсли НайденныеМатериалы1.Количество() Тогда
					ВыходныеИзделия1.Добавить(СтрокаТоваров);
				КонецЕсли;
		...
		Для Каждого Строка Из НайденныеМатериалы1 Цикл
			...
			СуммаСпецификации1 = СуммаСпецификации1 + (Строка.Цена * Строка.Количество);
		КонецЦикла;
		
		Материалы2 = Новый Массив;
		СуммаСпецификации2 = 0;
		Для Каждого Строка Из НайденныеМатериалы2 Цикл
			...
			СуммаСпецификации2 = СуммаСпецификации2 + (Строка.Цена * Строка.Количество);
		КонецЦикла;
		
		Если Не ЕстьМатериалы Тогда
			Если НайденныеМатериалы2.Количество() Тогда
				
				Для Каждого _СтрокаТоваров Из Товары Цикл
					СуммаИзделия = _СтрокаТоваров.Цена * _СтрокаТоваров.Количество;
					ПроцентСуммыИзделия = ОКР(СуммаИзделия/СтруктураПараметров.СуммаЗаказа * 100, 0);
					СуммаРазница = ОКР(СуммаСпецификации2 * (0.01 * ПроцентСуммыИзделия), 2);
					_СтрокаТоваров.Цена = (СуммаИзделия - СуммаРазница)/_СтрокаТоваров.Количество;
				КонецЦикла;
				
				СтрокаТоваров = Новый Структура;
				СтрокаТоваров.Вставить("Номенклатура", Материалы);
				СтрокаТоваров.Вставить("Цена", СуммаСпецификации2);
				СтрокаТоваров.Вставить("СуммаСкидки", 0);
				СтрокаТоваров.Вставить("Количество", 1);
				Товары.Добавить(СтрокаТоваров);
				ВыходныеИзделия2.Добавить(СтрокаТоваров); 
			...
			ИначеЕсли НайденныеМатериалы1.Количество() Тогда
				...
				Для Каждого _СтрокаТоваров Из Товары Цикл
					СуммаИзделия = _СтрокаТоваров.Цена * _СтрокаТоваров.Количество;
					ПроцентСуммыИзделия = ОКР(СуммаИзделия/СтруктураПараметров.СуммаЗаказа * 100, 0);
					СуммаРазница = ОКР(СуммаСпецификации1 * (0.01 * ПроцентСуммыИзделия), 2);
					_СтрокаТоваров.Цена = (СуммаИзделия - СуммаРазница)/_СтрокаТоваров.Количество;
				КонецЦикла;
				
				СтрокаТоваров = Новый Структура;
				СтрокаТоваров.Вставить("Номенклатура", Материалы);
				СтрокаТоваров.Вставить("Цена", СуммаСпецификации1);
				СтрокаТоваров.Вставить("СуммаСкидки", 0);
				СтрокаТоваров.Вставить("Количество", 1);
				Товары.Добавить(СтрокаТоваров);
				ВыходныеИзделия1.Добавить(СтрокаТоваров);
			...
			КонецЕсли;
		ИначеЕсли ЕстьМатериалы И Не ЕстьИзделие Тогда
			Если Не ВыходныеИзделия1.Количество() Тогда
				Для Каждого _СтрокаТоваров Из Товары Цикл
					Если _СтрокаТоваров.Номенклатура = Материалы Тогда
						_СтрокаТоваров.Количество = _СтрокаТоваров.Количество + 1;
						_СтрокаТоваров.Цена = ОКР(_СтрокаТоваров.Цена/_СтрокаТоваров.Количество, 2);
						
						СтрокаТоваров = Новый Структура;
						СтрокаТоваров.Вставить("Номенклатура", Материалы);
						СтрокаТоваров.Вставить("Цена", _СтрокаТоваров.Цена);
						СтрокаТоваров.Вставить("СуммаСкидки", 0);
						СтрокаТоваров.Вставить("Количество", 1);
						
						ВыходныеИзделия1.Добавить(СтрокаТоваров);
						
						Если ВыходныеИзделия2.Количество() Тогда
							ВыходныеИзделия2.Очистить();
		...
		//Если ЕстьМатериалы И Не ЕстьИзделие И Не ВыходныеИзделия1.Количество() Тогда
		//	
		//	Для Каждого _СтрокаТоваров Из Товары Цикл
		//		Если _СтрокаТоваров.Номенклатура = Материалы Тогда
		//			
		//			СтрокаТоваров = Новый Структура;
		//			СтрокаТоваров.Вставить("Номенклатура", Материалы);
		//			СтрокаТоваров.Вставить("Цена", 0);
		//			СтрокаТоваров.Вставить("СуммаСкидки", 0);
		//			СтрокаТоваров.Вставить("Количество", 1);
		//			
		//			ВыходныеИзделия1.Добавить(СтрокаТоваров);
		//			
		//			_СтрокаТоваров.Количество = 2;
		//			Прервать;
		//		КонецЕсли;
		//	КонецЦикла;
		//	
		...
		СтруктураПараметров.Вставить("ВыходныеИзделия1", ВыходныеИзделия1);
		СтруктураПараметров.Вставить("ВыходныеИзделия2", ВыходныеИзделия2);		
...
Функция ЗапросСпецификацииITOkna(МассивИдентификаторов)
	...
	|doc_acc_cnt_detals.zena_exp price,
...
Процедура СоздатьПакетДокументов(ПараметрыСозданияДокументов)
	...
	Для Каждого Параметр Из ПараметрыСозданияДокументов Цикл
		...
		КонецЦикла;
		ПараметрыОтбора = Новый Структура("УникальныйИдентификатор", Параметр.Ключ);
		НайденныеСтроки = СписокЗаказов.НайтиСтроки(ПараметрыОтбора);
		Если НайденныеСтроки.Количество() Тогда
	...
	ТекущийЭлемент = Элементы.СтраницаЗаказы1C;
...
Функция СоздатьИзменитьЗаказ(СтруктураПараметров)
	...
	ЗаказОбъект.Номер = СтрШаблон("%1-%2", СтруктураПараметров.Номер, Прав(Формат(Год(СтруктураПараметров.Дата), "ЧГ=0"),2));
...
Функция СоздатьПроизводствоБезЗаказа(СтруктураПараметров)
	...
	Если ТипЗнч(Материалы1) = Тип("Массив")
		...
		ВыходныеИзделияТЗ = ПроизводствоБезЗаказаОбъект.ВыходныеИзделия.Выгрузить();
		НоменклатураБуфер = Неопределено;
		НомерГруппыЗатрат = 0;
		Для Каждого Строка Из СтруктураПараметров.ВыходныеИзделия1 Цикл
			Если НоменклатураБуфер <> Строка.Номенклатура Тогда
				НомерГруппыЗатрат = НомерГруппыЗатрат + 1;
			КонецЕсли;
			СтрокаИзделие = ВыходныеИзделияТЗ.Добавить();
			...
			СтрокаИзделие.НомерГруппыЗатрат = НомерГруппыЗатрат;
			НоменклатураБуфер = СтрокаИзделие.Номенклатура;
		КонецЦикла;
		
		ВыходныеИзделияТЗ.Свернуть("Номенклатура,Подразделение,Получатель,НаправлениеВыпуска,НомерГруппыЗатрат",
		"Количество,КоличествоУпаковок");
		
		ПроизводствоБезЗаказаОбъект.ВыходныеИзделия.Загрузить(ВыходныеИзделияТЗ);
		
		МатериалыИРаботыТЗ = ПроизводствоБезЗаказаОбъект.МатериалыИРаботы.Выгрузить();
		Для Каждого Строка Из СтруктураПараметров.Материалы1 Цикл
			СтрокаМатериалы = МатериалыИРаботыТЗ.Добавить();
		...
		МатериалыИРаботыТЗ.Свернуть("Номенклатура,Подразделение,Склад,СтатьяКалькуляции,НомерГруппыЗатрат",
		"Количество,КоличествоУпаковок");
		
		ПроизводствоБезЗаказаОбъект.МатериалыИРаботы.Загрузить(МатериалыИРаботыТЗ);
	...
	Если ТипЗнч(Материалы2) = Тип("Массив")
		...
		НоменклатураБуфер = Неопределено;
		НомерГруппыЗатрат = 0;
		Для Каждого Строка Из СтруктураПараметров.ВыходныеИзделия2 Цикл
			Если НоменклатураБуфер <> Строка.Номенклатура Тогда
				НомерГруппыЗатрат = НомерГруппыЗатрат + 1;
			КонецЕсли;
			СтрокаИзделие = ВыходныеИзделияТЗ.Добавить();
...
Функция НоменклатураITOkna(Знач Артикул, Знач Наименование)
	...
	Если Артикул = "" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ".АртикулITOkna = &Артикул)", ".НаименованиеITOkna = &Наименование)");
		БылаЗаменаПараметра = Истина;
	КонецЕсли;
	Запрос.УстановитьПараметр("Артикул", Артикул);
	Запрос.УстановитьПараметр("Наименование", Наименование);
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() И Не БылаЗаменаПараметра Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ".АртикулITOkna = &Артикул)", ".НаименованиеITOkna = &Наименование)");
		РезультатЗапроса = Запрос.Выполнить();
	КонецЕсли;
...
Функция ЕстьЗаказITOkna(ИдЗаказаITOkna)
...
Процедура СохранитьДанные()
	...
	Настройки = СписокЗаказов1С.КомпоновщикНастроек.ПолучитьНастройки();
	
	ДанныеСохранения = Новый Структура;
	ДанныеСохранения.Вставить("НастройкиКД", Настройки);
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ИнтеграцияITOkna", Неопределено, ДанныеСохранения);
...
Процедура ЗагрузитьДанные()
	...
	Настройки = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ИнтеграцияITOkna", Неопределено);
	Если Настройки <> Неопределено Тогда
		СписокЗаказов1С.КомпоновщикНастроек.ЗагрузитьНастройки(Настройки.НастройкиКД);
	КонецЕсли;
